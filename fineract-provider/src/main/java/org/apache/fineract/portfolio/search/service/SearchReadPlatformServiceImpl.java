    ...
    private static final class SearchMapper implements RowMapper<SearchData> {
        ...
        public String searchSchema(final SearchConditions searchConditions) {
            ...
            final String clientMatchSql = " (select 'CLIENT' as entityType, c.id as entityId, c.display_name as entityName, c.external_id as entityExternalId, c.account_no as entityAccountNo "
                    ...
                    + " from m_client c left join m_office o on o.id = c.office_id left join `ADDITIONAL INFORMATION` a on c.id = a.client_id " +
                    " left join m_savings_account m ON c.id = m.client_id where o.hierarchy like :hierarchy and (c.account_no like :search or c.display_name like :search or c.external_id like :search or " +
                    "c.mobile_no like :search or a.`BVN ID` like :search or m.nuban_account_number like :search)) ";
